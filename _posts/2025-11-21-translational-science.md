...
이를 위해, 나는 PosterMate[^6]라는 시스템을 만들었다. PosterMate는 기업 리보트를 바탕으로 소비자를 구성하고, 소비자들을 마치 살아 움직이는 ‘페르소나’ 형식으로 만들어 페르소나의 시각을 시뮬레이션해 디자인 의사결정을 더 설득력 있게 만드는 툴이다. AI와 디자이너가 함께 기업 연구를 바탕으로 ‘다양한 관객’을 모의해 디자인을 다듬는 것이다.
