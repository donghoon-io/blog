...
다른 연구자들도 그렇듯, 한 사람이 한 분야의 모든 연구를 전부 다룰 수는 없다. 따라서 이 넓은 스펙트럼을 모두 포괄하기는 현실적으로 어려우며, 내가 집중한 영역은 다음과 같다 (내가 1저자로 진행한 연구들만 포함한다):
...
PaperTok은 “AI가 대신 만든 콘텐츠”가 아니라, 연구자가 ‘감독’처럼 AI가 만든 초안을 다듬는 새로운 협업 구조를 제시한다. 평가 결과, PaperTok으로 만든 영상은 기존 PDF-to-video 방식보다 훨씬 engaging했다. 이 연구는 학술 지식이 새로운 매체로 진화하는 방식을 제시하며, AI가 연구 커뮤니케이션의 미래에서 어떤 역할을 해야 하는지 질문을 던진다.
...
이를 위해, 나는 PosterMate[^6]라는 시스템을 만들었다. PosterMate는 소비자들을 마치 살아 움직이는 ‘페르소나’ 형식으로 만들어, 페르소나의 시각을 시뮬레이션해 디자인 의사결정을 더 설득력 있게 만드는 툴이다. AI와 디자이너가 함께 ‘다양한 관객’을 모의해 디자인을 다듬는 것이다.
